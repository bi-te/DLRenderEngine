...
float point_shadow_calc(float3 world_position, float3 light_pos, float3 normal, uint pLight_ind)
{
    float3 light_vec = normalize(light_pos - world_position);
    uint face = select_cube_face(-light_vec);

    float4x4 light_view_proj = g_lighting.pointTrans[pLight_ind].light_view_proj[face];
	
    world_position += light_vec * OFFSET;
    float4 compare = mul(light_view_proj, float4(world_position, 1.f));
    compare.z /= compare.w;

    float tex_size = 2.f * compare.w / g_lighting.buffer_side;

    world_position += normal * tex_size;
    float3 sample_dir = world_position - light_pos;

    return g_shadows.SampleCmp(g_comparison_sampler, float4(normalize(sample_dir), pLight_ind), compare.z);
}

float3 calc_point_light_pbr(PointLight pointLight, float3 position, float3 view_vec, float3 mesh_normal,
...
float3 calc_point_lights_pbr(float3 position, float3 view_vec, float3 mesh_normal, float3 normal, Material material)
{
    float3 color = 0.f;
    for (uint pLight_ind = 0; pLight_ind < g_lighting.pointLightNum; ++pLight_ind)
    {
        float depth = point_shadow_calc(position, g_lighting.pointLights[pLight_ind].position, mesh_normal, pLight_ind);
        color += depth * calc_point_light_pbr(g_lighting.pointLights[pLight_ind], position, view_vec, mesh_normal,
							normal, material);
    }
    return color;
...
float3 calc_spotlight_pbr(uint sLight_ind, float3 position, float3 view_vec, float3 mesh_normal, float3 normal, Material material)
{
    ...
    if (dot(light_vec, mesh_normal) <= 0.f)
        return 0.f;
...
float3 calc_spotlights_pbr(float3 position, float3 view_vec, float3 mesh_normal, float3 normal, Material material)
{
    float3 color = 0.f;
    for (uint sLight_ind = 0; sLight_ind < g_lighting.spotlightNum; ++sLight_ind)
    {
        ...
        color += depth * calc_spotlight_pbr(sLight_ind, position, view_vec, mesh_normal, normal, material);
    }
    return color;
}
