...
void LightSystem::bind_shadow_light(uint32_t index)
{
...
void LightSystem::bind_lights(LightBuffer* lBuffer)
{
	...
	for (uint32_t pLight = 0; pLight < lBuffer->pointLightNum; ++pLight)
	{
		...
		LightTransBuffer& trans = lBuffer->pointTrans[pLight];
		...
		trans.light_proj = perspective_proj(rad(90.f), 1.f, 0.1f, 100.f);
...
void LightSystem::init_depth_buffers(uint32_t side_size)
{
	...
	uint32_t lightsNum = pointLights.size();
	...
	texture_desc.ArraySize = lightsNum * 6u;
	...
	srv_desc.TextureCubeArray.NumCubes = lightsNum;
	...
	for (uint32_t light = 0; light < lightsNum; ++light)
	{
